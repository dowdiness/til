...
const tags = Array.from(new Set(items.flatMap((item) => item[1].tags))).filter(Boolean)

const tagRefs = Object.fromEntries(
  tags.map((tag) => [tag, items.filter(([_, schema]) => schema.tags?.includes(tag)).length]),
)
...
export const kableMain = () => {
  ...
  const unit = sine(200).add((x) => x.mul(0.8))
  console.log(unit)
