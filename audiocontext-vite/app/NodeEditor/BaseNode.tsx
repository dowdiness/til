...
export const BaseNode = memo(function BaseNode({ node, onNodeSelect, onConnectStart, onConnectEnd }: NodeElementProps) {
      ...
      <div className="flex flex-col items-center">
        <div className="flex w-full justify-evenly">
          {node.ins.map((_inlet, i) => {
            return <div className="connector" key={count++} onMouseDown={(e) => handleConnect(e, 'Top', i)} />
          })}
        </div>
        <div className="node" onMouseDown={handleNodeMouseDown}>
          {node.type}
        </div>
        <div className="connector" onMouseDown={(e) => handleConnect(e, 'Bottom', 0)} />
      </div>
    </div>
  )
})
